...
def load_balancing_loss(router_logits, tokens_per_expert, config: Config) -> torch.Tensor:
    ...
    scale_numerator = moe_num_experts * moe_loss_weight
...
class ParallelDroplessMLP(torch.nn.Module):
    def __init__(
        ...
        if use_bias:
            self.bias = torch.nn.Parameter(torch.empty(config.hidden_size))
...
class SparseGLU(SparseMLP):
    def __init__(
        ...
        super().__init__(config, parallel_config, parallel_context)
        self.w3 = ExpertParallel(
            nn.Linear(
                config.hidden_size,
                config.intermediate_size * self.experts_per_rank // self.tp_pg.size(),
        ...
        if self.tp_pg.size() == 1:
            self.w3.module.weight.data = self.w3.module.weight.data.T.contiguous()
